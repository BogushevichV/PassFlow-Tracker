	...
	public class TransportAnalytics
	{
			...
			DateOnly? dateTo = null)
		{
			...
			try
			{
				...
				{
					sql += " AND dr.id = ANY(@ids)";
					parameters.Add(new NpgsqlParameter("@ids", dailyRecordIds.ToArray()));
				}

				if (dateFrom.HasValue)
				{
					sql += " AND dr.record_date >= @dateFrom";
					parameters.Add(new NpgsqlParameter("@dateFrom", dateFrom.Value.ToDateTime(TimeOnly.MinValue)));
				}

				if (dateTo.HasValue)
				{
					sql += " AND dr.record_date <= @dateTo";
					parameters.Add(new NpgsqlParameter("@dateTo", dateTo.Value.ToDateTime(TimeOnly.MinValue)));
				}
			...
			DateOnly? dateTo = null)
		{
			...
			try
			{
				...
				if (dateFrom.HasValue)
				{
					sql += " AND record_date >= @dateFrom";
					parameters.Add(new NpgsqlParameter("@dateFrom", dateFrom.Value.ToDateTime(TimeOnly.MinValue)));
				}

				if (dateTo.HasValue)
				{
					sql += " AND record_date <= @dateTo";
					parameters.Add(new NpgsqlParameter("@dateTo", dateTo.Value.ToDateTime(TimeOnly.MinValue)));
				}
				...
				var duration = (DateTime.Now - startTime).TotalMilliseconds;
				AppLogger.Info($"[{LogContext}] Дни получены за {duration:F0}мс, записей: {data.Count}");
				return data;
			}
			catch (Exception ex)
			{
				AppLogger.Error($"[{LogContext}] Ошибка получения дней", ex);
				throw;
			}
		}

		// 5a. Cуммарный пассажиропоток по дням в заданном диапазоне
		public async Task<Dictionary<DateOnly, long>> GetDailyFlowAsync(DateOnly from, DateOnly to)
		{
			...
			const string sql = @"
				SELECT dr.record_date,
					   SUM(dr.entered + dr.exited) AS daily_flow
				FROM daily_records dr
				WHERE dr.record_date BETWEEN @from AND @to
				GROUP BY dr.record_date
				ORDER BY dr.record_date";

			using var cmd = new NpgsqlCommand(sql, conn);
			cmd.Parameters.AddWithValue("@from", from.ToDateTime(new TimeOnly(0, 0)));
			cmd.Parameters.AddWithValue("@to", to.ToDateTime(new TimeOnly(0, 0)));

			using var rdr = await cmd.ExecuteReaderAsync();
			while (await rdr.ReadAsync())
			{
				var date = (DateOnly)rdr["record_date"];
				var flow = Convert.ToInt64(rdr["daily_flow"]);
				result[date] = flow;
			}
